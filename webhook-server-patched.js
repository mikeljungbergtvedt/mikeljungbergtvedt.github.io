    ...
    req.on('end', async () => {
      ...
      if (forceClear) {
        try {
          ...
          const MEAS = '/Users/bot/peasy-auto/v2/logs.nosync/measurements.jsonl';
          if (fs2.existsSync(MEAS)) {
            const lines = fs2.readFileSync(MEAS, 'utf8').split('\n');
            let removed = 0;
            const kept = lines.filter(ln => {
              if (!ln.trim()) return false;
              try { const r = JSON.parse(ln); if ((r.regnr||'').toUpperCase() === regnr) { removed++; return false; } return true; } catch(_) { return true; }
            });
            fs2.copyFileSync(MEAS, MEAS + '.bak-' + Date.now());
            fs2.writeFileSync(MEAS, kept.join('\n') + (kept.length ? '\n' : ''));
            log(`[webhook] force-clear ${regnr}: fjernet ${removed} measurements-record(s)`);
          }

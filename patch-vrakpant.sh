#!/bin/bash
# Vrakpant-override i peasy-bot.js — fjerner d_lav_under_vrakpant-blocker,
# erstatter med override til (5000, 10000) og varsel "VRAKPANT-PRISING".
# Bilen prises og går til ERP normalt (ikke blokkert).
set -e

cd /Users/bot/peasy-auto/v2
BACKUP=peasy-bot.js.pre-vrakpant
...
# 1) Endre const dLav/dHoy til let + override før auctionTypeId
old = """    const brregObj = brregRes.brreg || brregRes || {};
    // 5. Skriv til ERP
    const dLav = pricing.dLav;
    const dHoy = pricing.dHoy;
    const auctionTypeId = dLav < 35000 ? 2 : 1;"""

new = """    const brregObj = brregRes.brreg || brregRes || {};
    // 5. Skriv til ERP
    let dLav = pricing.dLav;
    let dHoy = pricing.dHoy;
    // [vrakpant] Hvis dLav under norsk vrakpant → override til (5000, 10000) og send varsel
...
# 2) Fjern d_lav_under_vrakpant fra blockers (den er nå håndtert oppstrøms)
